...
This directory is **not** a production starter. It exists so we can `terraform apply` the module from a local checkout (GitHub Actions and local iteration).

- `source = "../../"` tests the code in this repo, not a published version.
- `backend "local"` keeps CI state on the runner. **Do not use a local backend in production.**
...
Within `examples/basic/` (eg, here), create a `terraform.tfvars` file with the following content, customizing AWS account id and Worklytics Tenant ID as needed.
